...
func _ready() -> void:
	...
	hud.web_beta_ui_override = true
	hud._apply_web_beta_ui()
	hud._configure_focus_cycles()

	_expect(not SpeechAdapter.is_available(), "Web no-voice mode reports speech unavailable")
	...
	_expect(not started and not SpeechAdapter.is_recording() and not GameState.interaction_time_frozen,
		"SpeechAdapter never starts recording or freezes time in no-voice mode")
	_expect(SpeechAdapter.http_request == null, "SpeechAdapter creates no HTTPRequest in no-voice mode")
	...
	_expect(hud.save_note.visible, "Pause menu shows the browser save note")
	_expect(hud.build_tag.visible, "Build tag is visible in web beta mode")

	var actions: Array = remap._active_actions()
	_expect(actions.size() == 14, "Remap panel exposes 14 actions in no-voice mode")
	...
	_expect(not SpeechAdapter.is_recording() and not SpeechAdapter.is_interaction_active(), "Speak key is ignored in no-voice mode")
	...
	if failures == 0:
		print("PASS: Stranger web-beta no-voice suite")
