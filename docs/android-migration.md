...
- [ ] **Phase 4 `:app` 外壳(2026-07-27 启动)** —— 原生应用壳。实施计划 `.claude/plans/scalable-brewing-wall.md`。里程碑:
  ...
  - **M3(先于 M2)**:**Play install-time Asset Pack 端到端投递验证** —— 唯一未跑通的发布链路(`ModelSource` seam 已留,见 `core-models/.../ModelSource.kt:36`)。`bundletool build-apks --connected-device` 本地装到模拟器,验首次启动 unpack 935MiB + warmup 校验 + analyze,全程无 adb push。**提前做** —— R-12 教训:"能 push 不等于能装",这是发布硬阻塞。
